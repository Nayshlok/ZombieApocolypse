...
namespace zombieApocalypse.Combat
{
    ...
    class CombatManager
    {
        ...
        public bool startCombat(Character attacker, Character defender)
        {
            ...
            else
            {
                ...
                if (confirmHit())
                {
                    ...
                    if (attacker is Zed)
                    {
                        damageHP = (((Zed)attacker).ZedClass == ZedClass.Shank) ? true : false;
                    }
        ...
        private void setAttackerBonuses()
        {
            ...
            {
                if (((Zed)attacker).ZedClass != ZedClass.Spitter)
                {
                    MeleePPBonus = ((attacker.PP - 14) / 2);
                    if (MeleePPBonus < 0)
                    {
        ...
        private bool confirmHit()
        {
            ...
            {
                if (((Player)defender).Class == ClassType.Warrior)
                {
                    AR += 2;
                }
            } 
        ...
        private Defense defenseChoice()
        {
            ...
            if (attacker is Zed)
            {
                tankAttack = (((Zed)attacker).ZedClass == ZedClass.Tank) ? true : false;
            }
        ...
        private bool pierceDamage()
        {
            ...
            isPierce = (attacker is Zed && ((Zed)attacker).ZedClass == ZedClass.Shank) ? true : false;
